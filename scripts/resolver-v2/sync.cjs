 ...
 *
 * Usage:
 *   node scripts/resolver-v2/sync.cjs [--dry-run] [--apply] [--auto-pr] [--draft] [--kind=<k>]
 *
 * Modes:
 ...
 *
 * Modifiers:
 *   --draft      with --auto-pr, open the PR as a DRAFT (resolver-plan v1.0
 *                Sprint 4). Used by the nightly resolver-catalog-sync GitHub
 *                Action so the unattended bot PR never auto-merges or pings
 *                reviewers until a human marks it ready. No-op without --auto-pr.
 *
...
function parseArgs(argv) {
  // `draft` is an additive modifier on --auto-pr (resolver-plan v1.0 Sprint 4):
  // it only changes whether `gh pr create` opens the PR as a draft. Default is
  // the historical behavior (non-draft). It is orthogonal to `mode`, so an
  // operator may combine `--auto-pr --draft` or set it alone (no-op unless
  // mode is auto-pr).
  const args = { mode: 'dry-run', kind: null, draft: false };
  for (const a of argv) {
...
/**
 * Pure builder for the ordered git+gh command sequence that --auto-pr executes.
 * Kept side-effect-free so the --draft threading is unit-testable without running
 * git/gh. `options.draft` toggles the `--draft` flag on the `gh pr create` command
 * only (additive; default = non-draft, the historical behavior).
 */
function buildAutoPrCommands(branch, written, options = {}) {
  const draftFlag = options.draft === true ? ' --draft' : '';
  return [
    `git checkout -b ${branch}`,
    `git add knowledge/recipients/`,
    `git commit -m "chore(resolver-v2): auto-sync catalog (${written} files updated)"`,
    `git push -u origin ${branch}`,
    `gh pr create${draftFlag} --title "chore(resolver-v2): auto-sync catalog" --body "Auto-generated by /resolver sync --auto-pr"`,
  ];
}

function runAutoPr(targetKinds, options = {}) {
  const draft = options.draft === true;
  console.log(`[resolver-v2 sync] AUTO-PR mode${draft ? ' (draft)' : ''}`);
  ...
  const commands = buildAutoPrCommands(branch, written, { draft });
  try {
...
function main() {
  ...
  try {
    if (args.mode === 'dry-run') {
      ...
      runAutoPr(targetKinds, { draft: args.draft });
    }
  } finally {
    releaseLock();
  }
...
module.exports = { parseArgs, diffCatalog, renderCatalog, runDryRun, runApply, runAutoPr, buildAutoPrCommands };
