 ...
 *
 * L2-CRITICAL validator for capability `image-platform`:
 *   - knowledge/image-adapters.yaml exists + parses + root is a mapping
 *   - each adapter: valid id (slug), unique; valid status enum
 *   - supports[] ∪ supports_stretch[] ∪ unsupported_warn[] ⊆ UNIVERSAL_PARAMS
 *     (kept in sync with scripts/image/lib/params.cjs — the single source of truth)
 *   - file-existence: status=installed ⇒ the `generator` path MUST exist on disk
 *     (registered-not-built stubs need NO file — model MATERIALIZED like validate-design-systems)
 *   - preset_of ⇒ the target adapter id exists
 *
 * Registered in BOTH scripts/check-consistency.cjs (local `pnpm check`) AND
 * .github/workflows/cross-tier-consistency.yml (CI runs a hand-picked subset, not
 * check-consistency.cjs — the two-edit rule). Exit 0 = pass, 1 = drift.
 */
...
const REGISTRY = path.join(REPO_ROOT, 'knowledge', 'image-adapters.yaml');

// Single source of truth for the universal param vocabulary (DRY with the runtime).
const { UNIVERSAL_PARAMS } = require('../image/lib/params.cjs');

const NAME_RE = /^[a-z][a-z0-9-]*$/;
const VALID_STATUSES = ['installed', 'registered-not-built'];
const MATERIALIZED_STATUSES = ['installed']; // only these require the generator file on disk.
...
function checkSubset(listName, list, tag, errs) {
  ...
  for (const p of list) {
    if (!UNIVERSAL.has(p)) errs.push(`${tag}: ${listName} contains "${p}" which is not a UNIVERSAL_PARAM (params.cjs)`);
  }
}

function main() {
  if (!fs.existsSync(REGISTRY)) {
    console.error('[FAIL] knowledge/image-adapters.yaml missing. (capability image-platform)');
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`[FAIL] image-adapters.yaml YAML parse error: ${e.message}`);
    process.exit(1);
  }
  if (doc === null || typeof doc !== 'object' || Array.isArray(doc)) {
    console.error('[FAIL] image-adapters.yaml root must be a mapping');
    process.exit(1);
  }
  const list = doc.adapters === undefined ? [] : doc.adapters;
  if (!Array.isArray(list)) {
    console.error('[FAIL] image-adapters.yaml: adapters must be an array');
    process.exit(1);
  }
  ...
  if (errs.length) {
    console.error(`[FAIL] image-adapters.yaml: ${errs.length} issue(s):`);
    for (const x of errs) console.error('  - ' + x);
    process.exit(1);
  }
  console.log(`[PASS] image-adapters.yaml (${list.length} adapter${list.length === 1 ? '' : 's'})`);
  process.exit(0);
}
