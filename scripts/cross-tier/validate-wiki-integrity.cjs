...
function checkLocalIntegrity() {
  ...
  for (const relPath of wikiFiles) {
    ...
    if (fm && typeof fm === 'object' && !Array.isArray(fm)) {
      ...
      // Frontmatter format:
      //   v3.0 entity-type-grouped: extracted_from_source: wiki/books/<slug>.md
      //   v4.0 source-grouped:      extracted_from_source: wiki/<source-slug>/source.md
      ...
      if (parts.length >= 3 && parts[0] === 'wiki') {
        ...
        // Exempt: wiki/_index/ (v4.0 reverse-lookup link-lists — derivative, regenerated by index-rebuild skill)
        if (parts[1] === '_index') continue;
        stats.findings.push({
          file: relPath,
          severity: 'warn',
