...
const CHARTS = Object.freeze({
  ...
  // comparison
  'small-multiples':      { family: 'comparison', built: false, fallback: 'bar', mckinsey: STANCE.PREFERRED, aka: ['trellis', 'panel'], note: 'Grid of simple charts, one per slice. (v-next renderer.)' },
  range:                  { family: 'comparison', built: false, fallback: 'dumbbell', mckinsey: STANCE.ACCEPTABLE, note: 'Range between two values per category.' },
  ...
  'matrix-chart':         { family: 'comparison', built: false, fallback: 'heatmap', mckinsey: STANCE.ACCEPTABLE, note: 'Presence/strength between two categorical sets.' },
  'word-cloud':           { family: 'comparison', built: false, fallback: 'bar', mckinsey: STANCE.AVOID, aka: ['tag cloud'], note: 'Word frequency; imprecise.' },
  'table-chart':          { family: 'comparison', built: false, fallback: 'heatmap', mckinsey: STANCE.ACCEPTABLE, note: 'Conditionally-formatted table.' },
  // correlation
  'connected-scatter':    { family: 'correlation', built: false, fallback: 'scatter', mckinsey: STANCE.ACCEPTABLE, note: 'Scatter with points linked in time order.' },
  hexbin:                 { family: 'correlation', built: false, fallback: 'scatter', mckinsey: STANCE.ACCEPTABLE, aka: ['hexagonal binning'], note: 'Density bins for huge scatters.' },
  ...
  waffle:                 { family: 'part-to-whole', built: false, fallback: 'stacked100', mckinsey: STANCE.ACCEPTABLE, note: 'Grid of squares as a part-to-whole.' },
  treemap:                { family: 'part-to-whole', built: false, fallback: 'bar', mckinsey: STANCE.ACCEPTABLE, note: 'Nested rectangles sized by value. (v-next renderer.)' },
  'population-pyramid':   { family: 'part-to-whole', built: false, fallback: 'diverging', mckinsey: STANCE.ACCEPTABLE, aka: ['age-sex pyramid'], note: 'Back-to-back bars by age/gender.' },
  ...
  sunburst:               { family: 'part-to-whole', built: false, fallback: 'bar', mckinsey: STANCE.ACCEPTABLE, note: 'Radial nested hierarchy.' },
  dendrogram:             { family: 'part-to-whole', built: false, fallback: 'bar', mckinsey: STANCE.ACCEPTABLE, note: 'Hierarchical clustering tree.' },
  venn:                   { family: 'part-to-whole', built: false, fallback: 'bar', mckinsey: STANCE.ACCEPTABLE, note: 'Set intersections.' },
  euler:                  { family: 'part-to-whole', built: false, fallback: 'bar', mckinsey: STANCE.ACCEPTABLE, note: 'Actual-overlap sets.' },
  ...
  'semicircle-donut':     { family: 'part-to-whole', built: false, fallback: 'donut', mckinsey: STANCE.ACCEPTABLE, note: 'Half-circle donut.' },
  // change over time
  bump:                   { family: 'change-over-time', built: false, fallback: 'line', mckinsey: STANCE.ACCEPTABLE, note: 'Ranking changes over periods.' },
  ...
  spline:                 { family: 'change-over-time', built: false, fallback: 'line', mckinsey: STANCE.ACCEPTABLE, note: 'Smoothed line.' },
  'step-line':            { family: 'change-over-time', built: false, fallback: 'line', mckinsey: STANCE.ACCEPTABLE, note: 'Right-angle steps for discrete intervals.' },
  gantt:                  { family: 'change-over-time', built: false, fallback: 'bar', mckinsey: STANCE.ACCEPTABLE, note: 'Project timeline bars. (v-next renderer.)' },
  candlestick:            { family: 'change-over-time', built: false, fallback: 'column', mckinsey: STANCE.ACCEPTABLE, note: 'Financial OHLC per period.' },
  ohlc:                   { family: 'change-over-time', built: false, fallback: 'column', mckinsey: STANCE.ACCEPTABLE, note: 'Open/high/low/close bars.' },
  barcode:                { family: 'change-over-time', built: false, fallback: 'column', mckinsey: STANCE.ACCEPTABLE, note: 'Vertical ticks for temporal anomalies.' },
  // distribution
  density:                { family: 'distribution', built: false, fallback: 'area', mckinsey: STANCE.ACCEPTABLE, note: 'Probability density curve.' },
  ridgeline:              { family: 'distribution', built: false, fallback: 'area', mckinsey: STANCE.ACCEPTABLE, note: 'Stacked density curves.' },
  violin:                 { family: 'distribution', built: false, fallback: 'box', mckinsey: STANCE.ACCEPTABLE, note: 'Mirrored density (box-plus-shape).' },
  strip:                  { family: 'distribution', built: false, fallback: 'scatter', mckinsey: STANCE.ACCEPTABLE, note: 'Individual values on one axis.' },
  jitter:                 { family: 'distribution', built: false, fallback: 'scatter', mckinsey: STANCE.ACCEPTABLE, note: 'Jittered categorical scatter.' },
  beeswarm:               { family: 'distribution', built: false, fallback: 'scatter', mckinsey: STANCE.ACCEPTABLE, note: 'Non-overlapping value swarm.' },
  horizon:                { family: 'distribution', built: false, fallback: 'area', mckinsey: STANCE.ACCEPTABLE, note: 'Compact banded temporal deviations.' },
  'radial-histogram':     { family: 'distribution', built: false, fallback: 'column', mckinsey: STANCE.AVOID, note: 'Polar histogram.' },
  // flow / geospatial
  sankey:                 { family: 'flow-geospatial', built: false, fallback: 'bar', mckinsey: STANCE.ACCEPTABLE, note: 'Flow quantities between nodes. (Needs graph layout — v-next.)' },
  chord:                  { family: 'flow-geospatial', built: false, fallback: 'heatmap', mckinsey: STANCE.ACCEPTABLE, note: 'Circular entity-to-entity links.' },
  arc:                    { family: 'flow-geospatial', built: false, fallback: 'scatter', mckinsey: STANCE.ACCEPTABLE, note: 'Linear nodes with arc links.' },
  network:                { family: 'flow-geospatial', built: false, fallback: 'scatter', mckinsey: STANCE.ACCEPTABLE, note: 'Node-link graph.' },
  flowchart:              { family: 'flow-geospatial', built: false, fallback: 'bar', mckinsey: STANCE.ACCEPTABLE, note: 'Process/decision diagram.' },
  choropleth:             { family: 'flow-geospatial', built: false, fallback: 'heatmap', mckinsey: STANCE.ACCEPTABLE, note: 'Regions colored by value. (Needs map topology — out of scope.)' },
  'tile-map':             { family: 'flow-geospatial', built: false, fallback: 'heatmap', mckinsey: STANCE.ACCEPTABLE, note: 'Equal-size grid map.' },
  'geo-heatmap':          { family: 'flow-geospatial', built: false, fallback: 'heatmap', mckinsey: STANCE.ACCEPTABLE, note: 'Spatial density map.' },
});
