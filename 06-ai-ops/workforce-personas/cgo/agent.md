# CGO agent — compiled output for `.claude/agents/cgo.md`

Copy block below verbatim into `.claude/agents/cgo.md`. SPEC remains here; drift caught by `pnpm check`.
...
- Frontmatter `name: cgo`.
- `tools` ⊆ `gtm-orchestrator.permissions` per ROLES.md.
- HITL max == C.
- Voice block keywords (`funnel stage`, `smallest test`, `kill threshold`) present.
- `routing_targets` from registry match playbook downstream list (cmo + cso Phase 2+).
