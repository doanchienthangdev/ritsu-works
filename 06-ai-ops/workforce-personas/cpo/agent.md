# CPO agent — compiled output for `.claude/agents/cpo.md`

Copy block below verbatim into `.claude/agents/cpo.md`. SPEC remains here; drift caught by `pnpm check`.
...
- Frontmatter `name: cpo`.
- `tools` ⊆ `product-orchestrator.permissions` per ROLES.md.
- HITL max == C.
- Voice block keywords (`name the stranger`, `N=10`, `never speculate`, `active beats passive`) present.
- Output contract sections present (User / Current behavior / Wedge fit / Success / Kill).
