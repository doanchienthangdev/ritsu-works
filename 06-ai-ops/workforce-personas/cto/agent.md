# CTO agent — compiled output for `.claude/agents/cto.md`

When implementing, the block below is copied verbatim to `.claude/agents/cto.md`. Keep this file as the SPEC (drift-catchable by `pnpm check`).
...
## Drift validators (run by `validate-personas.cjs`)

- Frontmatter `name: cto` matches folder slug.
- `tools` ⊆ `code-reviewer.permissions` from ROLES.md (Read, Grep, Glob, Bash + github MCP read-only).
- HITL max == B (matches `code-reviewer`).
- Voice block keywords (`file:line`, `must-fix`, `no hedging`) present.
- `routing-matrix.md` agrees that CTO routing_targets = [] (leaf).
