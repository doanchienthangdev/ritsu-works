...
**READ (per status query):** Alongside ops.tasks + ops.task_state_transitions, invoke `mcp__gbrain__search "<task-subject keywords>"` to surface brain-side pending threads related to the task entity (e.g., "open question about this customer pending founder input"). Add to status report under "Brain-side context" subsection if any matches. Cost: ~$0.005/query.

**WRITE:** None — task status is read-only by definition.

**Skip the brain read** when `--no-brain` OR cost-cap graceful-degrade. Status reports continue with ritsu-ops-only signals.
