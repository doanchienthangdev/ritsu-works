...
| ✅ native (v0.2) | `ref` reference-guided generation, `mask` inpaint region — via the `/v1/images/edits` multipart endpoint (`gen.cjs callOpenAiEdit`); single ref → `image`, multiple → `image[]` |
| ✅ composed (v0.2) | `style` (brand DESIGN.md tokens) + `art-style` (genre) — injected deterministically by `scripts/image/lib/compose.cjs` (no LLM); brand palette wins |
