...
| Class | Params |
|---|---|
| ✅ native | `ar` (flexible size), `tier` (→quality), `count` (n≤10), `format` (png/jpeg/webp), `style`, `art-style`, `enhance`, `safety` (→moderation), `max-cost-usd`, `dry-run`, `out`, `deck` |
...
| `--tier draft\|standard\|high` | `quality low\|medium\|high` + long-edge budget 1024/1024/2048 | `tierToQuality`. `high` also upsizes (1:1→2048², 16:9→2048×1152). |
...
## Generate
`node scripts/image/gen.cjs --prompt="<composed>" --ar=<> --tier=<> [--count --format --safety --style --art-style --max-cost-usd --out] [--dry-run]`
