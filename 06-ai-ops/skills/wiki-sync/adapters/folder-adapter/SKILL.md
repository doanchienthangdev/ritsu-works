...
wiki target for children: `wiki/<file-page-type>/<col-slug>__<file-slug>.md`
...
2. Compute file's slug: `file_slug = derived-from-frontmatter-or-filename`
3. Compute combined slug: `combined = <col_slug>__<file_slug>` (cap 80 chars)
4. Pre-check: `SELECT 1 FROM ops.knowledge_pages WHERE slug = combined`. If exists, log error + continue (partial-failure semantics).
5. Acquire `ops.wiki_sync_lock(<file-page-type>, combined)`. Release at COMMIT.
6. Invoke the sibling adapter normally, but inject:
   ...
   - `slug_override = combined` — bypasses adapter's default slug derivation
...
|---|---|---|---|---|---|
| 1 | attention.md | markdown | ✓ | papers__attention | $0.03 |
| 2 | cot.md | markdown | ✓ | papers__cot | $0.03 |
...
```jsonc
{
  ...
  "child_wiki_paths": ["wiki/concept/papers__attention.md", ...],
  "total_cost_usd": <sum>,
...
## Slug discipline (v2.0)

Per Hybrid B/A Tier C decision:
- Children use `<col-slug>__<file-slug>` in the global UNIQUE slug namespace
- The COLLECTION ITSELF does NOT get its own knowledge_pages row in v2.0
  (the folder-adapter's parent ingestion_jobs row is the only "collection" record)
- v2.1 may add a collection-index page (similar to chapter-splitter's `index.md` for books)
  + the `'collection'` page_type enum value
- Founder can discover children via `[[<file-page-type>/<col-slug>__<file-slug>]]` references
