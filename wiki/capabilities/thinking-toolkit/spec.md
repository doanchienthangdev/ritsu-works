...
**State:** operating (single-session ship 2026-05-28; … v2.1.0 grounded sell + dynamic-workflow 2026-06-04; v2.2.0 primary-source genre matrix + /dataviz 2026-06-04; v3.0.0 the consulting thinking-tool LIBRARY + fast checkpoint tool-selection 2026-06-05; **v3.0.1 20th toolkit #18 CX & Design Thinking folded in — 631-tool library + 20 domain processes 2026-06-05**; **v3.1.0** the 4S-diagram TOPOLOGY restored (3 paths + 5 decision-gates as first-class catalog sections) + a coherence validator (doc counts ↔ registries) + an auto-run enforcement hook at Sell + deeper before-sell checks — 2026-06-05; **v3.2.0** the INSTITUTIONALIZATION layer making `/think mckinsey` the workforce's PRIMARY problem-solver — the `problem-triage` router + `SOP-AIOPS-012` + the @ceo/@cgo/@cpo routing reflex — 2026-06-05; v3.3 Reasoning-Trace + Data-Verification; v3.4 the narrated engagement-journey trace; v3.5 toolkit-selection made VISIBLE; v3.6 per-checkpoint tool-selection; **v3.7.0 the CONSULT checkpoint — convene a `/muse` MASTER (David Ogilvy, Kotler, Schwartz, Munger, Voss, …) as a McKinsey team member on a craft/domain-mastery judgment, auto-self-play OR founder-seat, evidence-not-decider — 2026-06-08**)
**Proposed:** 2026-05-28
**Spec version:** 3.7.0 (header summary; **full per-version detail in `CHANGELOG.md`** — v3.0–v3.2 the consulting library + 4S topology + primary-solver institutionalization; v3.3–v3.6 reasoning-trace + verification + visible per-checkpoint tool-selection; **v3.7 the `/muse`-master CONSULT checkpoint**; §1–§13 describe the v1.0/v1.4 baseline)
...
## 18. v2.1 changelog (2026-06-04) — grounded sell templates + dynamic-workflow orchestration

`/cla extend thinking-toolkit` — MINOR (additive; `--workflow` defaults off → v2.0 preserved). @cto review APPROVE-WITH-NITS (4 fixes integrated). Two quality upgrades the founder asked for: sell *"đúng chuẩn McKinsey ngoài đời thực chứ không đoán"*, and dynamic workflows at the important points.

**Part 1 — ground the sell templates in REAL McKinsey reports (not "guessed").** A `researcher` fetched real McKinsey deliverables (the MGI *AI: The Next Digital Frontier* full report reproduction + verbatim deconstructions of *Women in the Workplace 2022*, *State of Fashion 2026*, etc.). v2.0's templates captured the *logic* (pyramid/MECE/SCQA) but MISSED the real "physical furniture": the boxed **IN BRIEF** (distinct from the exec summary), action titles on **EXHIBIT CAPTIONS** + a mandatory **source footer**, the named **"where to start"/"act now"** block, the **methodology/technical-appendix + survey-N**, the **city-tagged author box** + Preface. And a v2.0 BUG: "every section header is a full declarative sentence" mis-fits trend/survey pieces (which correctly use **topic-label** headers). Changes:
- NEW grounding reference `06-ai-ops/skills/thinking-toolkit/mckinsey-sell/mckinsey-deliverable-anatomy.md` — the 9 ordered real components + per-type skeletons + citations.
- `mckinsey-templates.yaml`: refined the 6 (folded in the In-Brief, exhibit-caption-source-footer, where-to-start; fixed the action-title rule to be genre-matched) + added **3 real publication types** (`mgi-research-report`, `insights-article` [topic-labels OK], `executive-briefing`) = **9 templates**.
- NEW **required field `grounded_in`** (citation array) on every template — schema `required` + a `validateEntry` non-empty-array branch — the mechanical **"not guessed" anchor**: a template cannot exist without a real-report citation. Optional `components` field for the report-type anatomy.

**Part 2 — `--workflow` dynamic-orchestration.** NEW param `--workflow=off`(default)`|steps|full`. When set, the high-leverage steps run as a **Claude Code dynamic Workflow** (the Workflow tool — many subagents in parallel) to push each step's quality far above a single-agent pass. **The load-bearing constraint** (from the workflow runtime): a workflow takes **no mid-run user input** → *"run each stage as its own workflow for sign-off between"* — which composes exactly with the checkpoint model: each **STEP runs as its own workflow** (heavy fan-out, no HITL inside); the **CHECKPOINTS between** stay in the active session for founder consensus. Workflow output is **evidence run through the validation gate**, not a decision (a fan-out judge-panel can't launder a verdict). Per-step patterns documented in SKILL.md (agent authors the workflow inline; pre-baked `.claude/workflows/` scripts deferred to v2.2): CP-HYPOTHESIZE → judge-panel of competing hypotheses; Solve → multi-modal sweep + adversarial-verify (mirrors `/deep-research`); CP-DISSENT → perspective-diverse red-team; Sell → judge-panel of drafts. A `workflow:<step>` annotation on the `checkpoint-log.md` row leaves an auditable trail (discipline-not-proof; not gated, since the run folder can't see the Workflow runtime).

**Touches:** `mckinsey-templates.yaml` (1.0.0 → 1.1.0; 6 → 9 + `grounded_in` on all) + its schema (`grounded_in` required + optional `components`) + `validate-mckinsey-templates.cjs` (`grounded_in` branch) + NEW `mckinsey-deliverable-anatomy.md` + `mckinsey-sell/SKILL.md` (cite the anatomy + the In-Brief / exhibit-caption-source-footer / where-to-start discipline + the 3 new types + "9 templates") + `mckinsey-workflow/SKILL.md` (the `--workflow` Dynamic-workflows section + the params table) + `think.md` (the mckinsey row) + resolver page regen. **Tests:** +6 `mckinsey-templates` (`grounded_in` branch) + the spec test now asserts 9 templates each carrying `grounded_in`. No DB table, no migration, no new `/think` verb.

**@cto must-fixes integrated:** the `GOOD()` test fixture gains `grounded_in` (else the happy-path/audience/root-boundary tests flip); `grounded_in` added to BOTH the schema `required` AND `validateEntry` (no split-brain); a dedicated `grounded_in` test `describe`; `evidence-not-decider` stated for workflow outputs; `mckinsey-sell/SKILL.md` "6 templates" → "9 templates"; the anatomy `.md` is a skill-internal file (NOT a resolver recipient — confirmed against the `eval-evo/playbooks/` precedent).

**Reversibility:** 5/5 — `git revert` removes the grounding doc + the 3 new templates + the `grounded_in` field + the `--workflow` section; the 6 v2.0 templates + the v2.0 engine are preserved. Decision: `ops.decisions` slug `thinking-toolkit-v2.1-grounding-and-workflows`.

## 19. v2.2 changelog (2026-06-04) — primary-source genre matrix + /dataviz integration

`/cla extend thinking-toolkit` — the founder's two upgrades: report presentation *"đạt tầm vóc mckinsey"* + integrate the new `/dataviz` capability. Grounded by the **`mckinsey-grounding-research` workflow** (founder-requested "tạo workflows") reading **3 REAL local McKinsey PDFs end-to-end** — [SIG] *Sustainable & inclusive growth 2025*, [AI25] *State of AI in 2025*, [B2B26] *Surprising economics of B2B growth 2026*.

**Part 1 — primary-source genre matrix.** v2.1's `mckinsey-deliverable-anatomy.md` was grounded in blog deconstructions; v2.2 rebuilds it from the 3 primary reports. **The key finding: McKinsey has NO single house format — it is a GENRE-KEYED format system.** Four genres (brand/impact · survey/data · thought-leadership-POV · decision-memo), each with its own **front-matter device** (visual-TOC + signed letter [brand] / numbered "Key findings" In-Brief [survey] / "At a glance" bold-italic box [POV]), **header style** (two-register topic-label+standfirst [brand] / action-title section AND exhibit [survey] / topic-label section + action-title exhibit [POV]), **close** (emotional+back-cover / prescribe-by-example paragraph / distributed action-bullets + urgency-coda), and **methodology placement** (footnote / "About the research" box / exhibit footnotes). `mckinsey-sell` now **classifies the genre FIRST**. Added the `pull_quote` + `Pro tip` elements and the two verbatim source-footer forms (analysis vs survey). `mckinsey-templates.yaml` re-anchored `grounded_in` to the 3 primaries (1.1.0 → 1.2.0). The v2.1 "always action-title for analytical/recommendation" rule was still too coarse — the full matrix fixes it (e.g. a [B2B26] POV report correctly uses topic-label *section* headers with action-title *exhibit* titles).

**Part 2 — `/dataviz` integration.** The new `/dataviz` capability (CLA A, shipped PR #235 — a McKinsey-caliber data-viz platform like `/image`) is now consumed at Sell: `mckinsey-sell` renders **every exhibit via `/dataviz`** (the exhibit's action-title is the chart `--message`+caption; the source line is the footer `/dataviz` mandates; `--style`/`--art-style` flow the brand — structure ⊥ brand). McKinsey is exhibit-led (a survey report is ~1 chart/page); the deliverable now ships genuine McKinsey-grade charts, not prose-described ones. The ghost-exhibit built at STRUCTURE names the chart's message + shape; Sell fills it with real data via `/dataviz`.

**Touches:** `mckinsey-sell/mckinsey-deliverable-anatomy.md` (rewritten — the primary-source genre matrix §0-§7) + `mckinsey-sell/SKILL.md` (genre-classify-first + the `## Exhibits via /dataviz` section) + `mckinsey-templates.yaml` (1.1.0 → 1.2.0; `grounded_in` re-anchored) + `mckinsey-workflow/SKILL.md` (Sell note). **No new gate, no schema change** (`grounded_in` already required v2.1). Tests: the existing `mckinsey-templates` spec test still asserts 9 templates each with `grounded_in`. Decision: `ops.decisions` slug `thinking-toolkit-v2.2-real-grounding-and-dataviz`. Reversibility 5/5.
