...
**Proposed:** 2026-05-28
**Spec version:** 3.2.0 (header summary; per-version detail in `CHANGELOG.md` — v3.0 integrates the `consulting-toolkit` corpus into the `/think mckinsey` checkpoint tool-selector; v3.0.1 adds the 20th toolkit; v3.1 restores the Cracked It! Fig 3.1 routing topology + closes the coherence gap + enforces the gate; v3.2 institutionalizes it as the primary solver (triage router + SOP-AIOPS-012 + persona reflex); §1–§13 describe the v1.0/v1.4 baseline)
