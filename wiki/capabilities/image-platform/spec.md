...
> **v0.4.0 (2026-06-02, `/cla extend` — founder: *"dùng chính skill `therichardngai-code/gpt-image-2-pro-max`, vẫn compose được với design-system / art-style / ritsu"*):** NEW backend **`--use=pro-max`** (long-form alias `gpt-image-2-pro-max`) — a **retrieval-augmented prompt engineer**. The upstream **MIT** skill is **vendored** (`vendor/gpt-image-2-pro-max/` — the `media-designer` agent + `search.py` + a `NOTICE.md`; its ChatGPT-OAuth / `gpt-image-1.5` companion generator is **deliberately not vendored**). `scripts/image/pro-max/search.cjs` is a Node thin client over the hosted, **free, key-less** BM25 corpus (~7,405 community prompts) at `goclawoffice.com` — registered in `knowledge/external-sources.yaml` (`gpt-image-2-prompts-backend`; same governance lane as a web search, non-sensitive briefs only). The in-session `image/enhance` skill gains a **pro-max mode**: *diagnose → search → pick a mood-aligned base → refactor into a parameterised template → resolve from intent*, with **mandatory author attribution** (`run.json.pro_max.base`). Generation then stays on our governed `gpt-image-2`, so **pro-max composes unchanged** with `--style` (brand lockup) + `--art-style` (genre) + `--ref`. New universal param `--enhance-mode`. **Graceful fallback** to the generic `--enhance` if the corpus is unreachable. **No new API key** (free backend + in-session refactor + existing out-of-band gpt-image-2). Governance was treated as **Tier C** (new external dependency + 3rd-party vendoring): documented inline + in `NOTICE.md`.
>
> **v0.3.1 (2026-06-02, `/cla extend` follow-up — founder: *"logo phải có kèm chữ và nhỏ thôi"* — the corner logo must be the **mark + wordmark lockup** like ritsu.ai, and smaller):** the overlay now stamps the brand's **canonical LOCKUP** (the pinwheel mark + the "Ritsu" cyan-gradient wordmark) instead of the bare mark, at a smaller size. NEW asset `00-core/design-system/ritsu/assets/ritsu-lockup.png` (799×237, transparent, ~3.4:1) — rasterized via **headless Chrome** from the mark + the Inter-Bold cyan-gradient wordmark (per §Logo), then cropped to content by the v0.3 PNG codec. DESIGN.md `logo.asset` → the lockup; `scale` 0.12 → **0.18** (the lockup is wide + short, so 18% width reads as a small, balanced nav-style logo). `compose.cjs` resolves `logo.asset` → an absolute `assetPath` (relative to the DESIGN.md dir); `gen.cjs` stamps **`policy.assetPath`** (the canonical lockup) — the `--ref` is the **trigger** (any ritsu asset works), with a fallback to the `--ref` when a policy declares no asset. `run.json.logo_overlay` gains `{asset, trigger_ref}`. 138 image tests.
>
> **v0.3.0 (2026-06-02, `/cla extend` — founder report: "ritsu-branded images put the logo too big and centered"):** **brand-scoped corner LOGO OVERLAY.** When a `--style` design system declares a `logo.overlay` policy in its DESIGN.md frontmatter **and** a `--ref` brand asset is given, `/image` stops feeding the square logo to the OpenAI *edits* endpoint (which reproduced it BIG + CENTERED) and instead: (1) generates a **clean base** via `/generations` — `compose.cjs` appends a `hasRef`-gated *"draw no logo, keep the corner clean"* directive to the brand block; (2) **composites the real ref asset small in the corner** via a new **dependency-free pure-Node PNG compositor** `scripts/image/lib/png-overlay.cjs` (zlib-only decode → alpha-weighted box downscale → "over" composite → encode; `sharp`/ImageMagick are absent and CI has no native-build job, so a from-scratch codec is the CI-safe choice). **Ritsu-scoped by data** (`00-core/design-system/ritsu/DESIGN.md` `logo:` block — `top-left`, `scale 0.12`, `margin 0.05`), **general by mechanism** (any design system may set the block). Overlay mode forces `/generations` + consumes the ref locally; **graceful fallback** (warn, keep the un-overlaid base) on any decode/encode/geometry failure; **PNG-only** (non-png → honest warn). `run.json` gains a `logo_overlay` block. **136 image tests** (was 87; +`png-overlay.test.ts` 31 + compose/gen overlay coverage). No Tier-2 migration. Authoritative current surface = `.claude/commands/image.md` + `06-ai-ops/skills/image/**` + `scripts/image/**`.
>
