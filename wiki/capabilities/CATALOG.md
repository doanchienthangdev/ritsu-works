...
| `image-platform` | foundational pluggable image-generation primitive (`/image`) — model-agnostic adapter layer (`--use`); gpt-image-2 real + pro-max preset + stubs; first-class `--style` design-system (deterministic DESIGN.md injection) + `--art-style` genre + `--ref`/`--mask` reference-guided generation (edits endpoint); reuses deepask helpers (untouched) | **0.2.0** | 06-ai-ops | 2026-06-02 | [spec.md](image-platform/spec.md) | [retrospective.md](image-platform/retrospective.md) |
