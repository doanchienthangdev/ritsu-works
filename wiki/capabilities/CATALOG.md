...
| `resolver-v3-jit-loading` | /resolver v3 — JIT Loading (Pocket Map + Drill-Down) | **3.0.2** | 06-ai-ops | 2026-05-25 (v3.0.0) / patched 2026-05-26 (v3.0.1 speed opts + v3.0.2 /resolver command doc-quality improvements via manual /evolve simulation) | [spec.md](resolver-v3-jit-loading/spec.md) | [retrospective.md](resolver-v3-jit-loading/retrospective.md) | (1) Tier C ceremony: CLAUDE.md cutover (16 `@imports` → 1 `@knowledge/recipients/INDEX.md`). (2) Tier C: activate `pre-bash-mass-action` + `pre-edit-significant` hooks in `.claude/settings.json`. (3) Apply migration 00038 via `supabase db push`. (4) 7-day baseline + canary verdict (`find()` > 20/day AND recall ≥ 17/20 nightly). See [retrospective §What still needs founder action](resolver-v3-jit-loading/retrospective.md). v3.0.1: pre-warm catalog + richer INDEX + Path A/B header. v3.0.2: /resolver command doc upgraded 57→78/100 via manual /evolve simulation (judge=proposer caveat; real /evolve to run after founder commits + rates HOLDOUT). |
