...
| `image-platform` | foundational pluggable image-generation primitive (`/image`) — model-agnostic adapter layer (`--use`); gpt-image-2 real + pro-max preset + nano-banana/midjourney/flux stubs; reuses deepask image helpers (untouched) + both style axes; A+ lean (run.json table-ready) | **0.1.1** | 06-ai-ops | 2026-06-02 | [spec.md](image-platform/spec.md) | [retrospective.md](image-platform/retrospective.md) |
