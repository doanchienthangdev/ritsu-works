...
## Layout: v4.0 source-grouped (added 2026-05-18)

Per `wiki/capabilities/wiki-sync-from-refs/spec.md` v4.0, distill+extract output uses source-grouped layout:

```
wiki/
├── <source-slug>/                          ← one folder per ingested source
│   ├── source.md                           ← source RECORD (frontmatter + body + pointer to raw/)
...
│   └── chapters/chapter-NN.md              ← page_type='book' slices (book chapters)
├── _index/                                 ← reverse-lookup link-lists (regenerated by index-rebuild skill)
│   ├── concept/<canonical-name>.md
│   ├── observation/<canonical-name>.md
│   ├── decision/<canonical-name>.md
│   └── idea/<canonical-name>.md
├── capabilities/                           ← capability specs (Phase 8 promoted)
└── (free-form notes — competitors/, market/, research/, etc. — unchanged)
```

**Skill input contract** (the founder anchor for v4.0):
- Whole package: `input = wiki/<source-slug>/`
- Type slice: `input = wiki/<source-slug>/concepts/`
- Single entity: `input = wiki/<source-slug>/concepts/<entity-slug>.md`

The composite UNIQUE on `(extracted_from_source_id, slug)` (migration 00032) means the same human-readable slug (e.g. `wedge`) can legitimately exist in multiple packages — different sources, different nuance, neither canonical.

**Voice-note / direct-intake flows** (`wiki/raw-ideas/<date>-<slug>.md`, `wiki/observations/<date>-<slug>.md`) are NOT v4.0-migrated — those flows have the founder as the implicit source, not an external document.
