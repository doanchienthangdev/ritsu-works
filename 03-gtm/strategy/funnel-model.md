...
- **Stage KPI:** `nps_very_disappointed_pct` (≥40%, owned by gtm-orchestrator) + Split the two: `day_30_retention` (≥30%, owner customer-lead) and `paid_retention_week_4` (≥25%, owner gtm-orchestrator) per kpi-ownership.yaml. The ≥30%+customer-lead framing is inherited from north-star §1.3, which itself contradicts the KPI registry; a reader cross-referencing the registry hits a mismatch. Either align to the registry or add a one-line note that the ≥30% traces to north-star while the registry's PMF-dissolution target for paid_retention_week_4 is ≥25%.. SOP-GTM-014 (Sean-Ellis survey) + SOP-METRICS-005 (cohort retention) instrument this.
