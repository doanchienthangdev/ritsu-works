...
> Customer funnel orchestration toward "100 paying customers who love Ritsu in 90 days." Composes modules from Marketing, Sales, Product, Customer, AI-Ops, Metrics. **This is a stage pillar — it dissolves when PMF criteria are met.**
...
This stage pillar owns the **integrated funnel** from first impression to first-month retention. It does NOT own the function modules themselves (those live in evergreen pillars). It owns the orchestration, the sequencing, the funnel-level decisions.

This pillar:
- **Drives** the "100 paying who love" goal (singular composite metric SOP-METRICS-007)
- **Sequences** end-of-stealth launch and post-launch experiments
- **Tests** which messaging + which channels actually convert
- **Coordinates** Collison-install handoff to Customer pillar
- **Dissolves** cleanly when PMF achieved — sub-pillars relocate per dissolution-target mapping below
...
`sops/SOP-GTM-NNN-<slug>/` — 17 SOPs scaffolded (see Phase 4 of migration-checklist). Phase 5 implements:
- `SOP-GTM-002-stealth-end-pre-launch-checklist` — runs ONCE before public launch
- `SOP-GTM-005-message-test-before-amplify` — gate before any AI multi-channel deploy
...
(Singular composite tile is `SOP-METRICS-007` in Metrics pillar; this pillar OWNS the underlying funnel KPIs.)
...
This pillar consumes modules from:
...
`SOP-GTM-005-message-test-before-amplify` is a `manual_gate` SOP. **No AI multi-channel deploy may run without a successful message-test pass.** This prevents the failure mode of spraying an untested message across 10 channels in parallel (AI amplifies bad messages as efficiently as good ones).

## Dissolution-target mapping

When PMF criteria met, sub-pillars relocate per `.archives/pillars/PLAN.md` §4.1:
...
This stage pillar is expected to live ~90 days from end-of-stealth (target dissolve: ~2026-08-15). If PMF not achieved by then, re-evaluate via `/muse:paul-graham` brainstorm — likely options: extend stage, pivot product, or wind down.
