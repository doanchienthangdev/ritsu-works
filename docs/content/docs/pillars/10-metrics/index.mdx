---
title: 10-metrics — Metrics Pillar
description: ''
source_path: 10-metrics/README.md
source_hash: e0f4ebb15f29aa0aee7e8bfeb361806fe0f395d5045470ac768c93ecf2623373
generated_at: '2026-05-19T05:20:46.422Z'
generated_by: docs-engine v1.1.0
category: pillar
language: vi
translated: true
translated_at: 2026-05-19T13:00:00Z
translated_by: "Claude Opus 4.7 (Claude Code Desktop, session translation)"
---

{/* generated-by: docs-engine v1.1.0 */}

# 10-metrics — Metrics Pillar

> KPI registry + ownership map, dashboards, alerting rules, PMF instrumentation, experiment measurement. The consolidation layer mà makes cross-pillar review possible. Without this, mỗi pillar's KPIs scatter.

**Layer:** Evergreen
**Stage status:** DEEP
**Pillar code:** METRICS
**Owner role(s):** metrics-curator, alert-router, experiment-analyst
**HITL baseline:** B (routine dashboards = B; new alert rule = C)

---

## Scope

Trụ cột này owns **measurement layer**:

- **KPI registry** — canonical list của every KPI in company, with formula, source, owner, target
- **PMF instrumentation** — Sean Ellis very-disappointed%, cohort retention, NPS, "100-paying-who-love" composite
- **Dashboards** — founder Monday dashboard (5 tiles single screen), cross-pillar weekly review, per-pillar health rollup
- **Alerting** — alert rule definitions, severity routing (P0 immediate Telegram → P1 daily digest), fatigue audit
- **Experiment measurement** — A/B test analysis, significance + lift calculation, stop-and-decide protocol
- **Revenue + cost views** — MRR/ARR/CAC/LTV + AI-Ops cost trends (read-only views over Finance + AI-Ops)

Trụ cột này KHÔNG sở hữu:
- The STORAGE substrate (`ops.kpi_snapshots`, `ops.alerts`, `metrics.product_dau_snapshot`) → those tables live in AI-Ops infrastructure (see `supabase/migrations/`)
- The DOMAIN MEANING của metrics (mỗi functional pillar owns its business interpretation)
- Decisions BASED ON metrics (những cái đó happen in owning pillar)

Metrics owns: definitions + dashboards + alerting routing. Giống like small data team sitting giữa AI-Ops storage và operational pillars.

## Sub-pillars (DEEP build now)

```
10-metrics/
├── kpi-registry/                         # Canonical metrics list + ownership
│   ├── sops/SOP-METRICS-001-kpi-definition-template/
│   ├── sops/SOP-METRICS-002-kpi-ownership-mapping/      # knowledge/kpi-ownership.yaml
│   └── sops/SOP-METRICS-003-kpi-deprecation-protocol/
├── pmf-instrumentation/                  # The PMF numbers
│   ├── sops/SOP-METRICS-004-sean-ellis-very-disappointed-tracking/
│   ├── sops/SOP-METRICS-005-cohort-retention-week-1-week-4/
│   ├── sops/SOP-METRICS-006-nps-collection-and-aggregation/
│   └── sops/SOP-METRICS-007-100-paying-who-love-composite-metric/   # The singular tile
├── dashboards/                           # Founder + cross-pillar views
│   ├── sops/SOP-METRICS-008-founder-monday-dashboard/   # 5-tile screen
│   ├── sops/SOP-METRICS-009-cross-pillar-weekly-review-board/
│   └── sops/SOP-METRICS-010-pillar-health-rollup/       # G/Y/R status per pillar
├── alerting/                             # Rule definitions + routing
│   ├── sops/SOP-METRICS-011-alert-rule-yaml-format/     # leverages knowledge/alert-rules.yaml
│   ├── sops/SOP-METRICS-012-alert-routing-by-severity/  # P0 → Telegram; P1 → daily digest
│   └── sops/SOP-METRICS-013-alert-fatigue-audit/        # Monthly review
├── experiment-measurement/               # A/B test rigor
│   ├── sops/SOP-METRICS-014-experiment-design-checklist/
│   ├── sops/SOP-METRICS-015-significance-and-lift-calculation/
│   └── sops/SOP-METRICS-016-experiment-stop-and-decide-protocol/  # ↔ 04-product/ab-test-discipline
└── revenue-and-cost-views/               # Read-only views over Finance + AI-Ops
    ├── sops/SOP-METRICS-017-mrr-growth-rate-weekly/
    ├── sops/SOP-METRICS-018-blended-cac-ltv-by-channel/  # ↔ 03-gtm/distribution-engine
    ├── sops/SOP-METRICS-019-ai-ops-cost-per-task-trend/  # ↔ 06-ai-ops/cost-budget-architecture
    └── sops/SOP-METRICS-020-runway-projection-monthly/    # ↔ 08-finance
```

## Sub-pillar order (narrative)

1. **kpi-registry** — canonical metric definitions + ownership map (foundation)
2. **pmf-instrumentation** — the PMF numbers: Sean-Ellis, cohort retention, "100 paying who love" composite tile
3. **dashboards** — founder Monday dashboard + cross-pillar weekly review board
4. **alerting** — rule definitions + severity-based routing (P0 Telegram, P1 daily digest)
5. **experiment-measurement** — A/B test rigor: design, significance, stop-and-decide
6. **revenue-and-cost-views** — read-only views over Finance + AI-Ops cost data

## SOPs

`sops/SOP-METRICS-NNN-<slug>/` — 20 SOPs scaffolded. Phase 5 implements:
- `SOP-METRICS-007-100-paying-who-love-composite-metric` — the singular PMF tile
- `SOP-METRICS-008-founder-monday-dashboard` — the screen displaying composite tile + 4 supporting tiles

## Agents

- `metrics-curator` (home) — owns KPI registry, weekly dashboard refresh
- `alert-router` (under `alerting`) — receives `ops.alerts` rows, routes per rules
- `experiment-analyst` (under `experiment-measurement`) — A/B test analysis on demand

## KPIs owned (meta — KPIs about KPIs)

- KPI freshness (% của registered KPIs updated weekly)
- Dashboard load time
- Alert P0 → action latency
- Alert fatigue ratio (alerts dismissed without action / total alerts)
- Experiment turnaround time (test launched → decision made)

## Dependencies

- **Composes from:** `06-ai-ops/cost-budget-architecture/` (cost trend feed), `08-finance/` (MRR + Stripe data), Supabase tables `ops.kpi_snapshots`, `ops.alerts`, `metrics.product_dau_snapshot`
- **Composed by:** every operational pillar reads metrics cho decisions; especially `03-gtm/pmf-instrumentation/` và `09-founder/weekly-review/`

## Critical PG gate

`SOP-METRICS-007-100-paying-who-love-composite-metric` là **singular tile** — one number combining paying count + retention + NPS into single PMF-progress score. Surfaces in founder Monday dashboard (`SOP-METRICS-008`). **Nếu number này doesn't move, nothing else matters.**

Inspired by PG's "weekly growth rate is the metric" — cho Ritsu's stage, "100-paying-who-love progress %" là equivalent.

## HITL baseline

- Routine dashboard refresh: Tier A
- New KPI registered: Tier B
- New alert rule (any severity): Tier C
- Alert routing change (eg moving P0 từ Telegram to email): Tier C
- KPI deprecation (removing từ registry): Tier C
- New SOP added: Tier C

## Entry conditions

Already DEEP. Future sub-pillars (post-1000-paying):
- `07-business-intelligence/` — deeper BI dashboards cho board reporting
- `08-data-warehouse/` — nếu Supabase strain on direct queries
