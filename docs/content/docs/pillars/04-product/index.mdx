---
title: 04-product — Product Operations Pillar
description: ''
source_path: 04-product/README.md
source_hash: d7c245abaef60f06b295af08527197571f4d5aac5d3b18a14d023437e56c902e
generated_at: '2026-05-19T05:20:46.417Z'
generated_by: docs-engine v1.1.0
category: pillar
language: vi
translated: true
translated_at: 2026-05-19T13:00:00Z
translated_by: "Claude Opus 4.7 (Claude Code Desktop, session translation)"
---

{/* generated-by: docs-engine v1.1.0 */}

# 04-product — Product Operations Pillar

> Build loop, user-listening pipeline, wedge-feature discovery, roadmap intake, A/B analysis. Đây là operations AROUND sản phẩm, KHÔNG là sản phẩm codebase (nằm ở một repo riêng backed by Product Supabase project `ritsu`).

**Layer:** Evergreen
**Stage status:** DEEP
**Pillar code:** PRODUCT
**Owner role(s):** product-orchestrator
**HITL baseline:** C (new features prioritized + experiments launched gate at C)

---

## Scope

Trụ cột này sở hữu:
- **Wedge-feature discovery** — which của 17+ activity types do real users come back for?
- **Build loop** — weekly feature prioritization, ship-then-listen cycle, rage detection from support
- **Feedback pipeline** — NPS, cancel-flow feedback, in-app feedback → roadmap intake
- **Pricing experiments** — A/B tests on tier boundaries (coordinates with `02-sales`)
- **A/B test discipline** — decision protocols, statistical rigor (coordinates with `10-metrics/experiment-measurement`)

Trụ cột này KHÔNG sở hữu:
- The product codebase (separate repo, Product Supabase)
- Customer onboarding flows (those execute in `05-customer/onboarding/`)
- Distribution / channel experiments → `03-gtm/distribution-engine/`
- Pricing tier definitions → `02-sales/modules/pricing-tiers/`

## Sub-pillars (DEEP build now)

```
04-product/
├── wedge-discovery/                          # N=10 strangers gate (PG)
│   ├── sops/SOP-PRODUCT-001-cofounder-usage-analysis/   # What do N=2 ACTUALLY use weekly?
│   ├── sops/SOP-PRODUCT-002-stranger-recruit-and-watch/ # 10 strangers, observed using
│   └── sops/SOP-PRODUCT-003-wedge-feature-naming-and-doubling-down/
├── build-loop/
│   ├── sops/SOP-PRODUCT-004-weekly-feature-prioritization/
│   ├── sops/SOP-PRODUCT-005-ship-then-listen-cycle/
│   └── sops/SOP-PRODUCT-006-rage-detection-from-support/
├── feedback-pipeline/
│   ├── sops/SOP-PRODUCT-007-in-app-nps-collection/
│   ├── sops/SOP-PRODUCT-008-cancel-flow-feedback/
│   └── sops/SOP-PRODUCT-009-feedback-to-roadmap-pipeline/
├── pricing-experiments/
│   ├── sops/SOP-PRODUCT-010-pricing-pull-test/
│   └── sops/SOP-PRODUCT-011-tier-boundary-experiment/
└── ab-test-discipline/
    └── sops/SOP-PRODUCT-012-ab-test-decision-protocol/
```

## Sub-pillar order (narrative)

1. **wedge-discovery** — N=10 strangers gate (PG critical_pg_gate). What works first.
2. **build-loop** — weekly prioritization + ship-then-listen cycle
3. **feedback-pipeline** — NPS, cancel-flow, in-product feedback → roadmap
4. **pricing-experiments** — pricing-pull test, tier-boundary experiments
5. **ab-test-discipline** — decision protocol cho mọi thứ experiment-shaped

## SOPs

`sops/SOP-PRODUCT-NNN-<slug>/` — 12 SOPs scaffolded. Phase 5 implements:
- `SOP-PRODUCT-001-cofounder-usage-analysis` — leverages existing N=2 cofounder data immediately

## Agents

- `product-orchestrator` (home) — weekly product review, feature prioritization
- `feedback-aggregator` (cross-cuts with `05-customer/feedback-and-research`) — pulls từ tất cả feedback sources
- `wedge-detector` (under `wedge-discovery`) — analyzes usage data cho 1-3 sticky features

## KPIs owned

(Surfaced in `10-metrics/kpi-registry/`.)

- Weekly active users (WAU)
- Magic moment completion rate (= aha moment percent)
- Feature usage distribution (17 activity types — which get used)
- Time-to-first-value (TTFV) — signup → first aha
- "Very disappointed without Ritsu %" by feature (PMF signal disaggregated)

## Dependencies

- **Composes from:** `00-charter/product.md` (canonical positioning), `02-sales/modules/pricing-tiers/` (pricing surface for experiments)
- **Composed by:** `03-gtm/funnel-orchestration/` (consumes wedge-discovery output), `05-customer/feedback-and-research/` (feeds back upstream)

## Critical PG gate

`SOP-PRODUCT-002-stranger-recruit-and-watch` là canonical N=10 strangers gate: **no major feature build proceeds without observed (not surveyed) usage từ 10 strangers (not cofounders, not friends).**

## HITL baseline

- Feature prioritization weekly review: Tier B
- New experiment launch: Tier C
- Pricing experiment with public surface change: Tier C (per HITL.md)
- New SOP added: Tier C

## Entry conditions

Already DEEP. No promotion needed. After PMF, có thể add `05-platform-and-extensibility` sub-pillar if API access becomes sticky product surface.
