---
title: 05-customer — Customer Pillar
description: ''
source_path: 05-customer/README.md
source_hash: 7369e8fb27d9c8977a4984b18ed5b536adae11e6653fe1881a18c61214ad631e
generated_at: '2026-05-19T05:20:46.418Z'
generated_by: docs-engine v1.1.0
category: pillar
language: vi
translated: true
translated_at: 2026-05-19T13:00:00Z
translated_by: "Claude Opus 4.7 (Claude Code Desktop, session translation)"
---

{/* generated-by: docs-engine v1.1.0 */}

# 05-customer — Customer Pillar

> Big pillar covering full customer lifecycle. Customer là danh từ (entity we serve); success/onboarding/support/retention/feedback/data là aspects của working with that noun. Matches `public.customers + persons + companies` schema (00005_customer_entity.sql).

**Layer:** Evergreen
**Stage status:** DEEP
**Pillar code:** CUSTOMER
**Owner role(s):** customer-lead, cs-coach, support-agent, retention-watcher, escalation-router, feedback-aggregator
**HITL baseline:** C (FAQ-handled replies = Tier B; non-FAQ = Tier C per HITL.md)

---

## Scope

Trụ cột này sở hữu **toàn bộ customer relationship** post-signup:

1. **Success** — does user achieve value? (activation, aha moment, mastery progression)
2. **Onboarding** — first-30-days experience (Collison install cho first 30 paying, AI takeover N=31+)
3. **Support** — reactive help khi things go wrong
4. **Retention** — proactive engagement to prevent churn
5. **Feedback & Research** — voice of customer (NPS, interviews, cancel-flow feedback)
6. **Customer Data** — CRM, customer-360, profile management (uses `public.customers`)

Trụ cột này KHÔNG sở hữu:
- The product itself → `04-product/`
- Distribution / acquisition channels → `03-gtm/`
- Billing / refund processing → `08-finance/`
- DMCA / GDPR enforcement → `07-trust-safety/` (data deletion mechanics are here, policy là there)

## Sub-pillars (DEEP build now)

```
05-customer/
├── success/                              # User achieves value (THE pillar's namesake)
│   ├── sops/SOP-CUSTOMER-001-aha-moment-definition-and-tracking/
│   ├── sops/SOP-CUSTOMER-002-activation-event-instrumentation/
│   ├── sops/SOP-CUSTOMER-003-day-1-day-7-day-30-cohort-tracking/
│   ├── sops/SOP-CUSTOMER-004-streak-and-emotional-hook-design/
│   └── sops/SOP-CUSTOMER-005-mastery-progression-tracking/    # Ritsu-specific
├── onboarding/                           # First 30 days
│   ├── sops/SOP-CUSTOMER-006-collison-install-script/         # Founder personally onboards first ~30 paying
│   ├── sops/SOP-CUSTOMER-007-stuck-user-detection-and-outreach/
│   ├── sops/SOP-CUSTOMER-008-onboarding-call-runbook/
│   └── sops/SOP-CUSTOMER-009-onboarding-script-automation-31plus/  # AI takes over N=31+
├── support/                              # Reactive
│   ├── sops/SOP-CUSTOMER-010-faq-classification-and-routing/
│   ├── sops/SOP-CUSTOMER-011-support-reply-drafting/
│   ├── sops/SOP-CUSTOMER-012-escalation-to-founder-criteria/
│   └── sops/SOP-CUSTOMER-013-incident-status-page-update/
├── retention/                            # Proactive
│   ├── sops/SOP-CUSTOMER-014-reactivation-email-on-7-day-silence/
│   ├── sops/SOP-CUSTOMER-015-cancel-flow-design/
│   ├── sops/SOP-CUSTOMER-016-win-back-attempt-protocol/
│   └── sops/SOP-CUSTOMER-017-deep-engagement-celebration-protocol/
├── feedback-and-research/                # Voice of customer
│   ├── sops/SOP-CUSTOMER-018-in-app-nps-collection/
│   ├── sops/SOP-CUSTOMER-019-cancel-flow-feedback-capture/
│   ├── sops/SOP-CUSTOMER-020-user-interview-recruitment/
│   └── sops/SOP-CUSTOMER-021-feedback-to-product-pipeline/   # ↔ 04-product/feedback-pipeline
└── customer-data/                        # CRM, 360, GDPR mechanics
    ├── sops/SOP-CUSTOMER-022-customer-360-refresh/
    ├── sops/SOP-CUSTOMER-023-gdpr-account-deletion/         # ↔ 07-trust-safety policy
    └── sops/SOP-CUSTOMER-024-segmentation-by-behavior/
```

## Sub-pillar order (narrative)

Customer lifecycle reads in two passes — chronological journey của one user, rồi cross-cutting "infrastructure" pillars mà serve all của chúng.

**Journey (user-time order):**
1. **onboarding** — first 30 days (founder personally onboards first ~30 paying)
2. **success** — user achieves value, "aha moment"; THE pillar's namesake
3. **support** — reactive resolution khi things go wrong
4. **retention** — proactive engagement, win-back, deep-engagement celebration
5. **feedback-and-research** — voice of customer through every channel

**Cross-cutting:**
6. **customer-data** — CRM/360 schema + GDPR mechanics (serves the other five)

## SOPs

`sops/SOP-CUSTOMER-NNN-<slug>/` — 24 SOPs scaffolded. Phase 5 implements:
- `SOP-CUSTOMER-006-collison-install-script` — script cho first 30 paying users

## Agents

- `customer-lead` (home, pillar-level) — owns weekly customer-health review
- `cs-coach` (under `success`) — activation funnel ownership
- `support-agent` (under `support`) — FAQ-handled tickets, escalation
- `retention-watcher` (under `retention`) — silence detection
- `escalation-router` (under `support`) — ticket → founder routing
- `feedback-aggregator` (under `feedback-and-research`) — voice of customer

## KPIs owned

(Surfaced in `10-metrics/kpi-registry/`.)

- Activation rate (signup → first aha)
- Day 7 / Day 30 retention (rolling cohorts)
- Support ticket volume by category
- % FAQ-handled vs human-needed (ratio over time)
- Churn rate (monthly)
- Win-back rate (cancelled → re-subscribed)
- NPS score (rolling)
- Mastery-progression rate (Ritsu-specific — % của users actually mastering content)

## Dependencies

- **Composes from:** `04-product` (wedge feature → activation), `09-founder` (founder personally onboards first ~30), `10-metrics` (PMF instrumentation feeds activation tracking), `public.customers + persons + companies + mv_customer_360`
- **Composed by:** `03-gtm/funnel-orchestration/` (consumes activation metrics), `07-trust-safety` (GDPR account-deletion uses `customer-data` mechanics)

## Critical PG gate

`onboarding/SOP-CUSTOMER-006-collison-install-script` là literal Collison-install protocol: **cho first ~30 paying users, founder does onboarding by Zoom in person, captures script verbatim, rồi `SOP-CUSTOMER-009-onboarding-script-automation-31plus` automates from N=31 onwards.** Do NOT let AI handle onboarding cho first 30 paying — những calls này produce wedge insight mà no other research captures.

## HITL baseline

- FAQ-handled support replies: Tier B (per ROLES.md `support-agent` definition)
- Non-FAQ support replies: Tier C
- Account suspension (any duration): Tier C
- GDPR data deletion: Tier D-Std (irreversible, requires founder approval)
- New SOP added: Tier C

## Entry conditions

Already DEEP. After 1000 paying users, `customer-data` có thể split into separate `account-management` sub-pillar.
