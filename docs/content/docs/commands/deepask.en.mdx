...
description: >-
  ...
  freshness-tagged answer in 1 of 12 formats — with McKinsey-grade charts embedded via /dataviz
  (v1.3) + intelligent illustrations via the /image platform (v1.4) where they make the point — OR
  an honest no-coverage gap + remedy. Internal-first (web leg delegated to deep-research). Tier A
  runtime (Tier-B+ legs surfaced, never auto-run).
source_path: .claude/commands/deepask.md
source_hash: d07364cc20fcaa8d7cdddfcd4db48dde25e7ac58efbde8418c275203853d05e9
generated_at: '2026-06-05T03:44:07.519Z'
...
| **v1.1.1 (extend)** | merged | **EXTRAORDINARY aesthetic bar** across ALL visual outputs — new skill `deepask/aesthetic` (references omgkit `/design:good` as floor, exceeds it; art-direction block for image-gen + polish checklist for code-render) wired into `deepask/format` §2.6 + `deepask/image-compose`. **+ LOGO FIX:** new `scripts/design-system/style-asset.cjs` embeds the `--style` brand logo as a base64 **data URI** (fixes `--format=html --style=ritsu` missing-logo / path bug). `/cla extend deepask`. |
| **v1.3 (extend)** | merged (#244) | **dataviz chart embedding** — the synthesis IR `charts[]` render via **`/dataviz`** (the repo's McKinsey-grade byte-stable SVG renderer) and embed in EVERY chart-carrying format: svg-inline (html/dashboard/interactive/canvas/article/pdf-via-html), PNG (pptx/docx, via headless-Chrome rasterize, graceful-degrade), **data-slide** (img-slide — a chart slide renders via dataviz, NOT gpt-image-2 which fabricates numbers), companion (infographics). LLM-native: the agent picks each chart from the dataviz catalog (`--selected-by=agent`). NEW `scripts/deepask/chart-embed.cjs` + `deepask/format` §2.7 + `--charts` flag. $0/offline (dataviz is pure). `/cla extend deepask`. |
| **v1.4 (extend)** | **this PR** | **/image platform integration** — ALL deepask image gen routes through the **`/image` platform** (`scripts/deepask/image-route.cjs` → `scripts/image/gen.cjs`) instead of the raw `image-gen.cjs`: pluggable adapter registry (`--use`), reference-guided gen (`--ref`/`--mask`), governed run.json + `ai-ops-image` cost-bucket + breaker. PLUS intelligent **concept illustration** (`--illustrate=hero|on`) for rich formats (`article`/`pdf`/`html`/`dashboard`) — agent-decided, explicit-only, budget-gated. `image-compose` still authors IR-grounded prompts (composed:true → no double-compose). NEW `scripts/deepask/image-route.cjs` + `deepask/format` §2.8 + `--illustrate`/`--use` flags. `/cla extend deepask`. |
...
| `--charts` | **auto** (default) · `off` | **(v1.3)** embed McKinsey-grade charts via `/dataviz` (`scripts/deepask/chart-embed.cjs`) wherever the IR has `charts[]` AND the format carries charts (svg-inline for html/dashboard/pdf/article; PNG for pptx/docx; **data-slides** for img-slide — accurate dataviz, not gpt-image-2; companion for infographics). The agent picks each chart type from the dataviz catalog (LLM-native, `--selected-by=agent`). **$0/offline** (dataviz is pure). `off` → no embedded charts. Brand `--style`/`--art-style` flow through to the charts. |
| `--illustrate` | **off** (default) · `hero` · `on` | **(v1.4)** add an intelligent **concept illustration** (hero/section — NOT a chart) via the **`/image` platform** (`scripts/deepask/image-route.cjs` → `scripts/image/gen.cjs`) to a rich format (`article`/`pdf`/`html`/`dashboard`/`interactive`/`canvas`) where it raises quality; the agent decides placement. **Explicit-only + `--max-cost-usd`-gated** (spends OpenAI $; `OPENAI_API_KEY` out-of-band). `off` → no illustrations. Orthogonal to `--charts` (DATA charts) — this is the CONCEPT-imagery axis. |
| `--use` | image adapter id · **gpt-image-2** (default) | **(v1.4)** the `/image` adapter for ALL deepask image gen (infographics/img-slide/illustration) — pluggable via `knowledge/image-adapters.yaml`. `--ref`/`--mask` (reference-guided gen) become available through `/image`. |
...
6. Format → **`inline` (default, no `--format`):** render the answer **into the conversation** (Pyramid + inline citations + Sources list; no files). **File mode (explicit `--format`):** artifact in `.archives/deepask/<YYYY-MM-DD>-<slug>/` (`answer.md` + `plan.json` + `sources.json` + rendered artifact). **`--style=<name>` (if given):** resolve once via `design-system` (`resolve-style.cjs`) and pass the tokens as **design context** into the format's renderer — no new dispatch row; non-visual formats → no-op (see `06-ai-ops/skills/deepask/format/SKILL.md` §1.5 + `SOP-AIOPS-007`).
   - **Charts (`--charts=auto`, v1.3):** for each IR `charts[]` entry the agent picks a chart type from the dataviz catalog (`06-ai-ops/skills/dataviz/catalog.md`, LLM-native) and `scripts/deepask/chart-embed.cjs` renders it via `/dataviz` ($0/offline), embedded per `format/SKILL.md` §2.7 — **svg-inline** (html/dashboard/pdf/article), **PNG** (pptx/docx), **data-slide** (img-slide: a chart slide via dataviz not gpt-image-2), **companion** (infographics). `--style`/`--art-style` flow to the charts. Enhancement only → render failure degrades, never fails the run.
