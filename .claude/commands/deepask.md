...
| `--art-style` | `<genre name>` · **omitted → no genre (plain)** | **ORTHOGONAL to `--style`; applies to BOTH image formats AND the code-rendered visual formats (v1.2.x).** Render in a named ARTISTIC GENRE from `knowledge/art-styles.yaml` (80 styles, e.g. `isometric-kingdom`, `scandi-minimal`, `risograph-print`). The genre owns composition (`layout`) + the illustration vocabulary (`assets`) + mood (`tone`) + a SECONDARY accent + display-headline character; the brand `--style` core palette / logo / body-type **ALWAYS WIN** on conflict. Resolved via `scripts/deepask/art-style.cjs` (`resolveArtStyle`; CLOSED registry → hard-fail on an unknown name, AD-3). For **image formats** (infographics/img-slide) `deepask/image-compose` injects it as the GENRE block between the brand block and `aesthetic` Part-3; for **code-rendered formats** (html/dashboard/interactive/canvas/chart + pdf-via-html) it drives the CSS aesthetic via `aesthetic` Part-2 (§1.5b). Non-visual/native (inline/text/article/xlsx/docx/pptx/mermaid) → honest no-op. This is the axis that turns "text on a gradient" into an art-directed, illustrated artifact. E.g. `--format=img-slide --style=ritsu --art-style=isometric-kingdom` or `--format=html --style=ritsu --art-style=neo-glassmorphism-3d`. |
| `--lang` | `<ISO code, e.g. vi>` · **omitted → IR-auto-detect** | **image formats (v1.2-image)** — when the rendered text is Vietnamese (or `--lang=vi`), `deepask/image-compose` appends a DIACRITIC-LOCKING sub-block (a VN-complete face — Be Vietnam Pro / Inter — + correctly-attached tone marks on ă â ê ô ơ ư đ; legibility outranks genre in the precedence order). |
...
6. Format → **`inline` (default, no `--format`):** render the answer **into the conversation** (Pyramid + inline citations + Sources list; no files). **File mode (explicit `--format`):** artifact in `.archives/deepask/<YYYY-MM-DD>-<slug>/` (`answer.md` + `plan.json` + `sources.json` + rendered artifact). **`--style=<name>` (if given):** resolve once via `design-system` (`resolve-style.cjs`) and pass the tokens as **design context** into the format's renderer — no new dispatch row; non-visual formats → no-op (see `06-ai-ops/skills/deepask/format/SKILL.md` §1.5 + `SOP-AIOPS-007`).
   - **Image branch (`infographics` · `img-slide`, v1.1 + v1.2-image):** `deepask/image-compose` turns the IR + the resolved `--style` (brand) tokens **+ the resolved `--art-style` genre** into an `image-plan.json` (per-piece gpt-image-2 prompts assembled as **brand STYLE block + GENRE block + a REQUIRED per-piece `focal_illustration` drawn from the cited IR + `aesthetic` Part-3 art-direction**, carrying the EXACT cited IR text). The orchestrator runs `image-cost.estimateRunCost` → `checkCostBudget(--max-cost-usd)` (REFUSE up front if over; always show the estimate), then `scripts/deepask/image-gen.cjs` per piece → `images/`/`slides/` PNGs, and for `img-slide` `scripts/deepask/slide-deck.cjs` → `slides.pdf` (cropped to true 16:9). `answer.md` is ALWAYS written alongside. **Honesty invariant (v1.2-image):** no load-bearing figure/claim may exist ONLY as illustration — every number stays legible text + cited; the illustration carries the SHAPE of an argument, never its SUBSTANCE. Flow: **analyze IR → split into cover/agenda/section/gaps/sources pieces → per piece: brand block + genre block + focal illustration + exact cited text → run gen → assemble PDF.**
