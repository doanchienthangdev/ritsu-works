...
| `--art-style` | `<genre name>` · **omitted → no genre (plain)** | **image formats only; ORTHOGONAL to `--style` (v1.2-image).** Render in a named ARTISTIC GENRE from `knowledge/art-styles.yaml` (80 styles, e.g. `isometric-kingdom`, `scandi-minimal`, `risograph-print`). The genre owns composition (`layout`) + the illustration vocabulary (`assets`) + mood (`tone`) + a SECONDARY accent + display-headline character; the brand `--style` core palette / logo / body-type **ALWAYS WIN** on conflict. Resolved via `scripts/deepask/art-style.cjs` (`resolveArtStyle`; CLOSED registry → hard-fail on an unknown name, AD-3). `deepask/image-compose` injects it as the GENRE block **between** the brand style block and the `aesthetic` Part-3 art-direction. This is the axis that turns "text on a gradient" into an art-directed, illustrated slide. E.g. `--format=img-slide --style=ritsu --art-style=isometric-kingdom`. |
