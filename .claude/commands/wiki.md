...
| `/wiki get --src=<spec> [--to=<path>] [--summary] [--include-frontmatter] [--max-entities=<N>]` | **v4.3 NEW.** Extract bundled context from a wiki source package as ONE markdown block. `<spec>` grammar: `<source-slug>` (full), `<source-slug>/chapter-N` (chapter + entities citing it), `<source-slug>/chapter-NN-<slug>` (specific chapter file), `<source-slug>/<type>/<entity-slug>` (one entity), `<source-slug>/<type>` (all of type). With `--to=<path>`, writes bundle to file; else prints to stdout. Use for: feeding chapter context to other commands (`/cla propose`, `@cgo`, `@cpo`) without duplicating into intermediate files. | A | read-only (logs to ops.agent_runs) |
...
Extract bundled context from a wiki source-grouped package. Output suitable for piping to another command's prompt OR writing to file via `--to=<path>`. **No LLM call** — pure filesystem read + assemble.
