...
1. **Detect adapter** — read `<path>`:
   - If directory → `folder-adapter` (v4.1: BFS-recursive bounded by `--depth=d` default 3, `--max-nodes=N` default 20. Each LEAF folder becomes its own `wiki/<col-slug>/` package; children at `wiki/<col-slug>/<child-slug>/source.md`).
