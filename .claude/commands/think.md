...
argument-hint: "[tosca <problem> | mece [list] | pyramid [conclusion] | so-what [conclusion] | 2x2 <axis-x> <axis-y> | driver-tree <metric> | list | flow [problem]]"
---

# /think

Project-scoped command for ritsu-works. Front-end for the **thinking-toolkit**
capability (v1.1.0). Capability spec at `wiki/capabilities/thinking-toolkit/spec.md`.

The command is a **thin orchestrator**. All thinking discipline lives in the 6
...
## Subcommands (v1.1 — 8 verbs)

| Invocation | Maps to skill | Purpose | HITL |
|---|---|---|---|
| `/think` | — | Show menu + composition flow + skill index | A |
| `/think list` | — | Table of all 6 thinking frameworks (name, one-line, when to use) | A |
...
Invoke `thinking-toolkit/tosca-problem-framing` skill with `<problem>` as input. The skill produces a 1-page T/O/S/C/A document. The agent writes the output to the conversation; founder may optionally `--to=<path>` capture to file (v1.2 — deferred).
...
- Output is by default in-conversation (no file writes). Future `--to=<path>` capture is deferred to v1.2 (revisit if founder demands).
