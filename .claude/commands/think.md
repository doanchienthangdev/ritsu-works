...
**v3.6 — `/muse` *inside* `/think mckinsey` (the `consult` checkpoint):** beyond the manual compose above, the McKinsey engine now convenes a `/muse` master **as a team member** at a craft/domain-mastery checkpoint (e.g. inviting David Ogilvy to a landing-page-rewrite study). The engine frames the craft need, picks the legend (`/muse:who` reasoning), runs the exchange — **you can take the client seat or let the engine self-play it** (`--consult=founder|auto`) — and takes the master's read through the validation gate as **evidence-not-decider** (a degree-6/7 expert judgment, triangulated vs the pulled data, never "the celebrity said so"). See `thinking-toolkit/mckinsey-consult`.
