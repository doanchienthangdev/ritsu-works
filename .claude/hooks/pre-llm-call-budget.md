...
version: 0.2.0  # v0.2.0 (2026-05-27, capability evolve v1.1 Sprint 1 sub-PR B): per_task_kind_caps tagged-unit dispatch ({unit: usd|messages, cap: N}); backward-compat with bare-number legacy = {unit: usd}; unknown unit fails open with audit
type: pre-tool
tools: [
  *_anthropic_*,
  llm_call,
...
function decide(payload):
  ...
  # v1.1 (capability evolve v1.1): caps may be tagged {unit, cap} or bare-number legacy.
  if task_kind:
    tk_caps = econ.get('per_task_kind_caps', {})
    tk_entry = tk_caps.get(task_kind)
    if tk_entry is not None:
      tk_unit, tk_cap = parse_tk_entry(role, task_kind, tk_entry)
      # parse_tk_entry handles three shapes:
      #   bare number    → ('usd', float) + logs ritsu.budget.legacy_cap_format event (one-time per role/task-kind/month)
      #   {unit, cap}    → (unit, cap)
      #   anything else  → (None, None) + logs ritsu.budget.malformed_cap_entry, allow-with-audit
      ...
      if tk_unit == 'usd':
        ...
        if (current_task_cost + estimated_call) > tk_cap * 2:  # 2x cap = hard escalate
          return escalate(
            ...
            log_extras={
              "task_kind_cap_unit": "usd",
              "task_kind_cap_value": tk_cap,
      ...
      elif tk_unit == 'messages':
        session_msg_count = get_session_message_count_for_task_kind(
          payload.session_id, task_kind
        )
        if (session_msg_count + 1) > tk_cap:  # strict: next call would breach
          return escalate(
            reason=f"task instance projected to exceed message-count cap for {task_kind}",
            log_extras={
              "task_kind_cap_unit": "messages",
              "task_kind_cap_value": tk_cap,
              "session_message_count": session_msg_count,
              "next_call_would_make": session_msg_count + 1,
            }
          )
      elif tk_unit is None:
        # parse_tk_entry already logged the malformed shape; fall through (no enforcement)
        pass
      else:
        # Unknown unit: fail-open with audit per spec §19.9. Safer than fail-closed
        # for unknown task kinds — operator may have added a new unit and forgotten
        # to update this hook; we want visibility, not breakage.
        write_event('ritsu.budget.unknown_unit', {
          'role': role,
          'task_kind': task_kind,
          'unit': tk_unit,
          'cap': tk_cap,
          'decision': 'allow (fail-open with audit)',
        })
...
## Per-task-kind cap schema (v0.2.0 — tagged units)

Capability `evolve` v1.1 introduced session-message-count caps alongside the
existing USD caps. To prevent silent unit collisions (the hook treating a
`{cap: 25}` message-count as `$25` and never firing), `per_task_kind_caps`
entries now MAY be tagged with their unit. Three accepted shapes:

```yaml
per_task_kind_caps:
  # Tagged form (v1.1+, preferred)
  eval-evo-iteration:                {unit: usd, cap: 0.50}
  eval-evo-skillopt-rollout-batch:   {unit: messages, cap: 25}

  # Bare-number legacy (v1.0; auto-treated as {unit: usd, cap: N}
  # and logged once per role/task_kind/month to ritsu.budget.legacy_cap_format)
  some-legacy-task: 0.10
```

**Dispatch rules** (per `parse_tk_entry` in the decision logic above):

| Cap shape | Enforcement | Comparison |
|---|---|---|
| bare number `N` | `unit: usd` | `current_task_cost + estimated_call > 2 × N` → escalate |
| `{unit: usd, cap: N}` | USD | same as above |
| `{unit: messages, cap: N}` | session messages for this task_kind | `session_msg_count + 1 > N` → escalate |
| `{unit: <other>, cap: N}` | none (fail-open) | logs `ritsu.budget.unknown_unit`, allows the call |
| malformed (missing `cap`, etc.) | none (fail-open) | logs `ritsu.budget.malformed_cap_entry`, allows |

**Why fail-open on unknown unit:** an operator added a new cap shape to
`governance/ROLES.md` for a task_kind this hook doesn't yet understand. Failing
closed would block legitimate work for every task of that kind. Failing open
with an audit trail (`ops.events`) gets the operator visibility on the next
weekly cost review without surface-area disruption.

**Migration discipline:** mixed-shape `per_task_kind_caps` blocks are
intentionally supported during the v1.0 → v1.1 transition window. The L2
validator `governance/ROLES.md ↔ skills` does NOT enforce tagged form. A
follow-up `pnpm migrate:roles-caps` (not in this Sprint) will offer to
batch-convert remaining bare numbers when the workforce has stabilized on
the tagged form.

**`session_message_count` definition:** counts subagent dispatches (via Task)
within the current session that carry the same `task_kind` metadata. Tracked
per-process; resets on session start. The session bridge (sub-PR C) is
responsible for tagging each `Task()` invocation with the correct task_kind
...
| 11 | Task instance estimated to exceed 2x its task_kind cap (usd) | escalate |
...
| 15 | `per_task_kind_caps[X] = 0.50` (bare number) for task_kind X, call would project to $1.20 | escalate + log `ritsu.budget.legacy_cap_format` (once per role/task_kind/month) |
| 16 | `per_task_kind_caps[X] = {unit: messages, cap: 25}`, session has dispatched 24 same-task_kind subagents, next call is 25th | allow (25 ≤ 25) |
| 17 | same as #16 but next call is 26th | escalate |
| 18 | `per_task_kind_caps[X] = {unit: messages, cap: 25}`, no session_message_count exists yet | allow (treats as 0) |
| 19 | `per_task_kind_caps[X] = {unit: gpu_hours, cap: 5}` (unknown unit) | allow + log `ritsu.budget.unknown_unit` |
| 20 | `per_task_kind_caps[X] = {cap: 10}` (missing unit field) | allow + log `ritsu.budget.malformed_cap_entry` |
